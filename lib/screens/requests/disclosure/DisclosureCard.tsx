            ...
            <Container flexDirection={"row"} padding>
              <Container flex={1} paddingRight>
                <Button
                  ...
                  onPress={() =>
                    requestModel.cancelButton.action(requestModel.requestId)
                  }
                />
              </Container>
              <Container flex={2}>
                <Button
                  disabled={
                    !!requestModel.error || requestModel.actionButton.disabled
                  }
                  buttonText={requestModel.actionButton.text}
                  block={Button.Block.Filled}
                  type={Button.Types.Primary}
                  onPress={() =>
                    requestModel.actionButton.action(
                      requestModel.requestId,
                      requestModel.actionButton.actionType
                    )
                  }
                />
              </Container>
        ...
        <Container>
          <Banner
            ...
            avatar={
              ...
              //requestModel.appBranding.profileImage
            }
              ...
              {requestModel.requestItems.map((item: any, index: number) => {
                return (
                  <ListItem
                    key={item.key}
                    title={item.type}
                    last={requestModel.requestItems.length - 1 === index}
                  >
                    {item.value}
                  </ListItem>
                );
          ...
          {requestModel.verifiedCredentials.length > 0 && (
            <Section noTopBorder noTopMargin>
              {requestModel.verifiedCredentials.map(
                (vc: any, index: number) => {
                  return (
                    <Credential
                      key={vc.iss + "-" + index}
          ...
          {requestModel.missingCredentials.length > 0 && (
            <Section noTopBorder noTopMargin>
              {requestModel.missingCredentials.map(
                (item: any, index: number) => {
                  return (
                    item.essential && (
                      <Credential
                        key={item.claimType + "-" + index}
