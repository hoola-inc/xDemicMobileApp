...
const parseErrorMessage = (message: string) => {
  return message.startsWith("JWT has expired")
...
const countStats = (stats: { [key: string]: number }) => {
  ...
  typeof stats === "object"
    ? Object.keys(stats).forEach(value =>
        value !== "request" ? (total += stats[value]) : null
      )
    : null;
  return total;
};
 ...
 */
const DisclosureRequestModel = (
  props: any
): DisclosureRequestModelType | null => {
  /**
  ...
  const disclosureRequestCommon = {
    ...
    appBranding: {
      profileImage: Images.profile.avatar,
    ...
    statsMessage: interactionStatsMessage(
