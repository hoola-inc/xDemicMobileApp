...
function processStatusReducer(state = hashMap(), action) {
  switch (action.type) {
    case SAVE_MESSAGE:
      return assocIn(
        updateIn(state, [action.section], section => dissoc(section, "error")),
        [action.section, "message"],
        action.message
      );
    case SAVE_ERROR:
    ...
    case STOP_WORKING:
      return updateIn(state, [action.section], section =>
        dissoc(section, "working")
      );
    case COMPLETE_PROCESS:
      return updateIn(state, [action.section], section =>
        assoc(dissoc(section, "working", "message"), "completed", true)
      );
    case FAIL_PROCESS:
      return updateIn(state, [action.section], section =>
        assoc(dissoc(section, "working", "message"), "error", action.error)
      );
    default:
      return state;
  }
