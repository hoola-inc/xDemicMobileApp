...
} from "xdemic/lib/constants/UportActionTypes";
import { IMPORT_SNAPSHOT } from "xdemic/lib/constants/HubActionTypes";
...
import { massage } from "xdemic/lib/sagas/stateSaver";
  ...
  switch (action.type) {
    ...
    case STORE_IDENTITY:
      ...
      return assoc(
        assocIn(
          state,
          ["byAddress", action.identity.address],
          merge(existing, toClj(action.identity))
        ),
        "currentIdentity",
    ...
    case ADD_OWN_CLAIMS:
      ...
      return updateIn(state, ["byAddress", action.address, "own"], claims =>
        merge(claims, toClj(action.claims))
      );
    case REMOVE_OWN_CLAIM:
      const claims = getIn(state, ["byAddress", action.address, "own"]);
      if (get(claims, action.claimType)) {
        return assocIdentity(
          state,
          action.address,
          "own",
          dissoc(claims, action.claimType)
        );
      } else {
        return state;
      }
    ...
    case ADD_RECOVERY_ADDRESS:
      return updateIn(state, ["byAddress", action.address], id =>
        merge(
          id,
          toClj({
            recoveryAddress: action.recoveryAddress,
    ...
    case STORE_ENCRYPTION_KEY:
      return updateIn(state, ["byAddress", action.address], id =>
        merge(id, toClj({ publicEncKey: action.publicEncKey }))
      );
    case STORE_PUTUTU_TOKEN:
      return updateIn(state, ["byAddress", action.address], id =>
        merge(id, toClj({ pututuToken: action.pututuToken }))
      );
    ...
    case SAVE_META_NONCE:
      return assocIdentity(state, action.address, "metaNonce", {
        nonce: action.nonce,
    ...
    case SAVE_BALANCE:
      return assocIdentity(state, action.address, "balance", {
    ...
    case REMOVE_IDENTITY:
      return updateIn(
        get(state, "currentIdentity") === action.address
          ? dissoc(state, "currentIdentity")
          : state,
        ["byAddress"],
        identities => dissoc(identities, action.address)
      );
    case STORE_EXTERNAL_UPORT:
      return updateIn(state, ["external", action.address], profile =>
        merge(profile, toClj(action.profile))
      );
    case STORE_ACTIVITY:
      return assocIn(
        state,
        [
          ...
          "activities",
          action.activity.id.toString()
        ],
        toClj(action.activity)
      );
    case UPDATE_ACTIVITY:
      ...
      if (activityPair) {
        return assocIn(
          state,
          ["byAddress", activityPair.address, "activities", action.activityId],
          merge(activityPair.activity, toClj(action.changes))
        );
      } else return state;
    case OPEN_ACTIVITY:
      activityPair = findActivity(state, action.activityId);
      if (activityPair) {
        return assocIn(
          state,
          ["byAddress", activityPair.address, "activities", action.activityId],
          assoc(activityPair.activity, "opened", true)
        );
      } else return state;
    case REMOVE_ACTIVITY:
      ...
      if (activityPair) {
        return updateIn(
          state,
          ["byAddress", activityPair.address, "activities"],
          activities => dissoc(activities, action.activityId)
        );
      } else return state;
    ...
    case STORE_ATTESTATION:
      ...
      return assocIn(
        state,
        ["byAddress", action.attestation.sub, "attestations", tokenHash],
        toClj(action.attestation)
      );
    case REMOVE_ATTESTATION:
      return updateIn(
        state,
        ["byAddress", action.address, "attestations"],
        attestations => dissoc(attestations, action.attestationHash)
      );
