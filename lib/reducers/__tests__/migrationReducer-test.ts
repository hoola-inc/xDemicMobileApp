...
populated.steps[MigrationStep[MigrationStep.MigrateLegacy]] =
  MigrationStatus.Started;

describe("addMigrationTarget", () => {
  it("adds a target", () => {
    expect(
      reducer(initialState, addMigrationTarget(MigrationTarget.Legacy))
    ).toMatchSnapshot();
  });

  it("does not add an existing target", () => {
    expect(
      reducer(populated, addMigrationTarget(MigrationTarget.Legacy))
    ).toEqual(populated);
  });
});
...
describe("startedMigrationStep", () => {
  it("starts a step", () => {
    expect(
      reducer(initialState, startedMigrationStep(MigrationStep.MigrateLegacy))
    ).toMatchSnapshot();
  });

  it("does not a step if already started", () => {
    expect(
      reducer(populated, startedMigrationStep(MigrationStep.MigrateLegacy))
    ).toEqual(populated);
  });
...
describe("completedMigrationStep", () => {
  it("completes a step", () => {
    expect(
      reducer(initialState, completedMigrationStep(MigrationStep.MigrateLegacy))
    ).toMatchSnapshot();
  });

  it("completes a step if already started", () => {
    expect(
      reducer(populated, completedMigrationStep(MigrationStep.MigrateLegacy))
        .steps[MigrationStep.MigrateLegacy]
    ).toEqual(MigrationStatus.Completed);
  });
...
describe("failedMigrationStep", () => {
  it("fails a step", () => {
    expect(
      reducer(initialState, failedMigrationStep(MigrationStep.MigrateLegacy))
    ).toMatchSnapshot();
  });

  it("fails a step if already started", () => {
    expect(
      reducer(populated, failedMigrationStep(MigrationStep.MigrateLegacy))
        .steps[MigrationStep.MigrateLegacy]
    ).toEqual(MigrationStatus.Error);
  });
