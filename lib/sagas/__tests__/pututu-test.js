...
import { handleURL, handleMessage } from "xdemic/lib/actions/requestActions";
import { updateBadgeCount } from "xdemic/lib/actions/snsRegistrationActions";
import { storePututuToken } from "xdemic/lib/actions/uportActions";
import { offline } from "xdemic/lib/selectors/processStatus";

import pututuSaga, {
  fetchAndHandleEncryptedMessage,
  makeRequest,
  clearMessageData,
  handleEncryptedMessage,
  fetchMessageData,
  getOrCreatePututuAuthToken,
  pollForMessages,
  checkAndHandleExistingMessages
} from "../pututu";
import { decryptMessage, decryptLegacy } from "xdemic/lib/sagas/encryption";
import { createPututuAuthToken, verifyToken } from "xdemic/lib/sagas/jwt";
...
const testData = [
  {
    ...
    message:
      '{"ciphertext": "xdP3d9e9sQhTB4FP/Omf3N68GRSb5u/vHlkhFUTcj+58UjQ6aBJc/DLjG3ArNS/UPpO+XnKB/Dc8tXram6Xc6OD5Qpnyfn/txoqPudc0XkM=", "ephemPublicKey": "oGZhZ0cvwgLKslgiPEQpBkRoE+CbWEdi738efvQBsH0=", "nonce": "/20Dn2PowBiZu5kw42pO7F/wiKvkecM3", "version": "x25519-xsalsa20-poly1305"}',
  ...
  },
  {
    ...
    recipient: "2ooGueTmPCkNBtzUYZSimzPRLNpqHWhCtj3",
    message:
      '{"from":"FVYw0qlV3w5otw9js/mgmWMLBnOUDlR1AcHwlj97DAg=","nonce":"1xdoW0pUuXKgEwNcFupagSd9d/QRtLUJ","ciphertext":""}',
    ...
    return expectSaga(handleEncryptedMessage, testData[0])
      .provide([
        ...
        [
          call(
            decryptMessage,
            JSON.parse(testData[0].message),
            testData[0].recipient
          ),
          JWT
        ],
      ...
      return expectSaga(handleEncryptedMessage, testData[1])
        .provide([
          ...
          [
            call(
              decryptLegacy,
              JSON.parse(testData[1].message),
              testData[1].recipient
            ),
            `{"url":"${url}"}`
          ],
...
describe("checkAndHandleExistingMessages", () => {
  ...
  it("does not fetch without an address", () => {
    return expectSaga(checkAndHandleExistingMessages)
      .provide([[select(currentAddress), undefined], [select(offline), false]])
      .not.call(fetchMessageData, "")
      .run();
  });
});
...
describe("fetchMessageData", () => {
  it("makes a request and interprets response", () => {
    ...
    return expectSaga(fetchMessageData, id)
      .provide([[matchers.call.fn(makeRequest), response]])
...
describe("clearMessageData", () => {
  it("makes a request to clear data from pututu", () => {
    ...
    return expectSaga(clearMessageData, id)
      .provide([[matchers.call.fn(makeRequest), undefined]])
