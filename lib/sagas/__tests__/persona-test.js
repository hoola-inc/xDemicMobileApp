...
import { web3ForAddress } from "xdemic/lib/selectors/chains";
import {
  addClaims,
  saveIpfsProfile,
  updateActivity,
...
import { addData } from "xdemic/lib/actions/onboardingActions";
...
import { saveShareToken } from "xdemic/lib/actions/myInfoActions";
import { working } from "xdemic/lib/selectors/processStatus";
import { connected } from "../networkState";
import { createToken } from "../jwt";
import { addImage, ipfsUrl, addJson } from "xdemic/lib/utilities/ipfs";
...
import {
  ...
  setUportUrl,
  registry,
  savePublicUportToIPFS,
  addImageToIpfsOnboarding
} from "../persona";
  ...
  describe("using did", () => {
    ...
    it("fetches with profile", () => {
      return expectSaga(fetchPublicUport, did)
        .provide([
          [
            call(resolve, did),
            { ...profile, uportProfile: { name: "Bob Smith" } }
          ]
        ])
        .call(resolve, did)
    ...
    it("fetches and returns simple while translating images", () => {
      return expectSaga(fetchPublicUport, did)
        .provide([
          [
            call(resolve, did),
            {
              ...
              uportProfile: {
                name: "Bob Smith",
                image: { contentUrl: "/me.jpg" },
                banner: { contentUrl: "/banner.jpg" }
              }
            }
          ]
        ])
        ...
        .returns({
          name: "Bob Smith",
          avatar: { uri: "https://cloudflare-ipfs.com/me.jpg" },
          banner: { uri: "https://cloudflare-ipfs.com/banner.jpg" }
        })
        .run();
    });
...
describe("savePublicUportToIPFS", () => {
  ...
  it("successfull call", () => {
    ...
    const profile = {
      publicKey:
        "0xf09ef47d5359e36d8fb004be6bf96591836fcdf8421fe599f7e33fd97fc8d0746bbd509d00c90a8c5860bb097e4911b133d405238d67f524d892f6c13dd9b7dc",
      publicEncKey:
        "0xf19ef47d5359e36d8fb004be6bf96591836fcdf8421fe599f7e33fd97fc8d0746bbd509d00c90a8c5860bb097e4911b133d405238d67f524d892f6c13dd9b7dc"
    };
...
describe("savePublicUport", () => {
  ...
  it("Does not start process second time", () => {
    return expectSaga(savePublicUport, { address })
      .provide([[select(working, "persona"), true]])
      .returns(false)
      .run();
  });
  ...
  it("fresh identity", () => {
    return expectSaga(savePublicUport, { address })
      .provide([
        ...
        [
          call(handleURL, {
            url: setUportUrl(address, base58ToHex(ipfsHash)),
            postback: false,
            popup: false
          }),
          { id: 13123 }
  ...
  it("unchanged identity", () => {
    return expectSaga(savePublicUport, { address })
      ...
      .put(startWorking("persona"))
      .not.put(
        saveMessage("persona", "Updating DID Document in uPort Registry")
      )
      .put(completeProcess("persona"))
      .returns(true)
      .run();
...
describe("setUportUrl", () => {
  it("handles legacy uport address", () => {
    expect(
      setUportUrl(
        ...
        base58ToHex("QmdNm7VZTNnux2LS6vMJXLZ5BGb3rPJV9mF5jA72uq9y8o")
      )
    ).toEqual(
      "me.xdemic:0xb9C1598e24650437a3055F7f66AC1820c419a679?function=setAttributes(bytes%200x1220df69b4a5bc0ddd8fa4c17d25e035b1c29816c4cab26cdff802a3f1880bbfdca4)&label=uPortRegistry"
    );
  });
  ...
  it("handles rinkeby address", () => {
    expect(
      setUportUrl(
        "2oVdmcz7BkWozm2JE4hHixRV8s5y3STqhPG",
        base58ToHex("QmdNm7VZTNnux2LS6vMJXLZ5BGb3rPJV9mF5jA72uq9y8o")
      )
    ).toEqual(
      "me.xdemic:2oKVhUttUcwaFAopRBGA21NDJoYcBb3a6iz?function=set(bytes32%200x75506f727450726f66696c654950465331323230000000000000000000000000%2C%20address%200xb08e78b8e17dc2874818d7f49055abf08ee9977d%2C%20bytes32%200xdf69b4a5bc0ddd8fa4c17d25e035b1c29816c4cab26cdff802a3f1880bbfdca4)&label=uPortRegistry"
    );
  });
  ...
  it("handles kovan address", () => {
    expect(
      ...
      "me.xdemic:354S1QuCzkmKoQ3ADSLp1KtqAe8gZ74F9am?function=set(bytes32%200x75506f727450726f66696c654950465331323230000000000000000000000000%2C%20address%200x00521965e7bd230323c423d96c657db5b79d099f%2C%20bytes32%200xdf69b4a5bc0ddd8fa4c17d25e035b1c29816c4cab26cdff802a3f1880bbfdca4)&label=uPortRegistry"
    );
  });
  ...
  it("rejects unsupported addres", () => {
    ...
    ).toThrow(`unsupported address: 5A8bRWU3F7j3REx3vkJWxdjQPp4tqmxFPmab1Tr`);
  });
});
...
describe("addImageToIpfsOnboarding", () => {
  ...
  it("stores image and saves it to own claims", () => {
    return expectSaga(addImageToIpfsOnboarding, {
      avatarObj: { avatar: image }
    })
      .provide([[call(connected), true], [call(addImage, image), ipfsHash]])
      .call(addImage, image)
      .put(
        addData({
          avatar: {
            uri:
              "https://cloudflare-ipfs.com/ipfs/QmdNm7VZTNnux2LS6vMJXLZ5BGb3rPJV9mF5jA72uq9y8o"
          }
        })
      )
      .run();
  ...
  it("does not store data if saving fails", () => {
    return expectSaga(addImageToIpfsOnboarding, {
      ...
      .provide([[call(connected), true], [call(addImage, image), undefined]])
      .call(addImage, image)
      .not.put(
        addData({
          avatar: {
