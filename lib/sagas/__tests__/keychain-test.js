...
jest.mock("react-native-uport-signer", () => {
  ...
  return MockNativeModule(
    ...
    privateKey,
    publicKey,
    "0x7f2d6bb19b6a52698725f4a292e985c51cefc319"
  );
});
...
import {
  ...
  incIdentityIndex,
  incAccountIndex
} from "xdemic/lib/actions/HDWalletActions.js";
import { storeEncryptionKey } from "xdemic/lib/actions/uportActions";

import { saveError } from "xdemic/lib/actions/processStatusActions";
import {
  autoPrompt,
  loadRawKey,
  saveKey,
...
import naclutil from "xdemic/lib/vendor/nacl-util";
...
const publicKey =
  "0x0403fdd57adec3d438ea237fe46b33ee1e016eda6b585c3e27ea66686c2ea5358479";
...
describe("createIdentityAddress()", () => {
  describe("without existing seed", () => {
    it("creates new seed", () => {
      return (
        expectSaga(createIdentityAddress)
          .provide([[select(hdRootAddress), undefined]])
    ...
    it("creates and stores key", () => {
      ...
      return expectSaga(createIdentityKeyPair, 1)
        .provide([
          [select(hdRootAddress), undefined],
          [
            call(encryptionPublicKey, { idIndex: 1, actIndex: 0 }),
            ENC_KEY_PAIR.public
          ]
        ])
        .put(storeIdentity(identity))
    ...
    it("Returns keypair", () => {
      return expectSaga(encryptionKeyPair, { idIndex: 1, actIndex: 2 })
        .provide([
          ...
          [
            call(
              RNUportHDSigner.privateKeyForPath,
              address,
              `m/7696500'/1'/2'/2'`,
    ...
    it("Returns keypair", () => {
      return expectSaga(encryptionKeyPair, { idIndex: 1, actIndex: null })
        .provide([
          ...
          [
            matchers.call.fn(loadRawKey, ENC_KEY_PAIR.public),
            ENC_KEY_PAIR.secret
          ]
        ])
        .returns(KP)
...
describe("encryptionSecretKey()", () => {
  ...
  describe("account path values are not yet set", () => {
    it("returns secret key", () => {
      return expectSaga(encryptionSecretKey, {
        idIndex: undefined,
        actIndex: undefined
      })
        .provide([
          ...
          return expectSaga(canSignFor, address)
            .provide([
              [
                ...
                { hexaddress: deviceAddress, hdindex: 1 }
              ],
        ...
        it("returns false", () => {
          return expectSaga(canSignFor, address)
            .provide([
              [
                select(networkSettingsForAddress, address),
                { address: deviceAddress, hdindex: 1 }
              ],
              [select(isHD, address), true],
              [select(hdIdentityIndex, address), 0],
              [select(hdAccountIndex, address), 1],
