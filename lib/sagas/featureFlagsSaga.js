...
import { FEATURE_FLAGS_LOAD } from "xdemic/lib/constants/FeatureFlagsActionTypes";
...
export function loadFromFirebase() {
  ...
  return firebase
    .config()
    .fetch(config.flags.transport.firebase.cacheDuration)
    .then(() => {
  ...
  if (!currentChannel) {
    try {
      ...
      yield call(
        setUserProperty,
        "channel",
        config.flags.transport.firebase.defaultChannel
      );
...
function* featureFlagsSaga() {
  yield all([takeEvery(FEATURE_FLAGS_LOAD, loadRemoteFeatureFlags)]);
}
