...
import { randomBytes } from "xdemic/lib/helpers/random_bytes";
...
export function* encryptMessage(message, boxPub) {
  ...
  const { publicKey, secretKey } = nacl.box.keyPair.fromSecretKey(
    naclutil.decodeBase64(secret.toString("base64"))
  );
  const nonce = yield cps(randomBytes, 24);
  const padded = pad(message);
  const ciphertext = nacl.box(
    naclutil.decodeUTF8(padded),
    naclutil.decodeBase64(nonce.toString("base64")),
    naclutil.decodeBase64(boxPub),
    secretKey
  );
  return {
...
export function* encryptEvent(event, address) {
  const accountIndex = yield select(hdAccountIndex, address);
  const secret = yield call(encryptionSecretKey, {
    idIndex: 0,
    actIndex: accountIndex
  });
  const nonce = yield cps(randomBytes, 24);
