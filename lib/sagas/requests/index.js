...
import { fetchRequest, currentRequestId } from "xdemic/lib/selectors/requests";
import {
  currentAddress,
  selectedIdentity
...
import requestScreenManager from "xdemic/lib/utilities/requestScreenManager";
...
export function findRequestType(parsed) {
  if (parsed.protocol === "ethereum:") {
    ...
    parsed.protocol === "me.xdemic:" ||
    (parsed.protocol === "https:" && parsed.host === "id.uport.me")
  ) {
...
export function* handleURL({
  ...
  try {
    ...
    if (parsed.query.redirect_url) {
      ...
      if (
        redirect &&
        redirect.protocol !== undefined &&
        callback &&
        redirect.protocol.match(/(http|https):/)
      ) {
        if (redirect.hostname !== callback.hostname) {
          yield put(
            saveError(
              "handleUrl",
              "Redirect URL and Callback URL must have matching host names"
            )
          );
          return;
        }
...
export function* performCallback(request, response) {
  ...
  const redirectUrl = request.redirect_url
    ? URL(request.redirect_url)
    : undefined;

  if (
    request.postback !== false ||
    callbackUrl.hostname.match(/^(chasqui|api)\.uport\.(me|space)$/)
  ) {
...
export function* authorizeAndCallback(request) {
  ...
  if (handler && handler.authorize) {
    ...
    if (request.infoType === "seedRecovery") {
      try {
        yield put(
          updateActivity(request.id, { authorizedAt: new Date().getTime() })
        );
...
export function* selectAccount({ activityId, account }) {
  ...
  if (
    request.network &&
    account &&
    MNID.decode(account).network === request.network
  ) {
    ...
    if (accountRecord) {
      yield put(
        updateActivity(activityId, {
          account,
          target: accountRecord.parent || account
        })
      );
    }
  }
