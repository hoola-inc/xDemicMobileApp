...
import { failProcess } from "xdemic/lib/actions/processStatusActions";
import {
  endpointArn,
  skippedPushNotifications
} from "xdemic/lib/selectors/snsRegistrationStatus";
...
import { checkRpcNetwork } from "xdemic/lib/sagas/blockchain";
import {
  accountsForNetwork,
  hasPublishedDID,
  publicEncKey,
...
} from "xdemic/lib/selectors/attestations";
...
export function* handle(payload, jwt) {
  ...
  if (payload.claims) {
    ...
    if (payload.claims.verifiable) {
      yield put(
        track("requestVerifiedClaim", {
          claim: Object.keys(payload.claims.verifiable).join(","),
  ...
  try {
    ...
    yield put(
      updateInteractionStats(request.target, request.client_id, "request")
    );
...
export function* findCorrectAccount({
  target,
  client_id,
  network,
  actType,
  ...
  if (!((networks[network] || {}).supported || (network && rpc))) {
    throw new Error(`xDemic does not support ${networkName} at the moment`);
  }
  if (actType !== "keypair" && !networks[network].relayUrl) {
    throw new Error(
      `xDemic does not support smart contract accounts on ${networkName} at the moment`
    );
  }
  ...
  if (rpc) {
    ...
    if (checked !== network)
      throw new Error(
        `App requested an account on chain ${network}, but endpoint returned ${checked}`
      );
    base.rpc = rpc;
  }
  ...
  if (actType === "segregated") {
    ...
    throw new Error(
      "App requested a segregated account, but did not pass in a client_id"
    );
    ...
    throw new Error(
      "App requested a segregated keypair account, but did not pass in a client_id"
    );
  }
...
export function* disclosureRequest(basicReq, parsed) {
  try {
    ...
    if (request.actType !== "none" && request.actType !== "general")
      request.accountAuthorized = false;
    const account = yield call(findCorrectAccount, request);
    const current = yield select(networkSettings);
...
export function* createNewAccount({
  ...
  const params = {
    parent: target,
    network: (networks[network] || { name: network }).name
  };
  if (rpc) {
    params.rpcUrl = rpc;
  }
...
export function* authorizeDisclosure(request) {
  try {
    ...
    const issuer =
      !legacyClient && !request.target.match(/did:/)
        ? `did:uport:${request.target}`
        : request.target;
    ...
    if (verified && verified.length > 0) {
      ...
      yield call(
        authorize,
        `Do you want to share your information with ${counterparty}?`
      );
      const claimNames = (request.claims
        ? Object.keys(request.claims.verifiable || {})
        : request.verified
      ).join(",");
      yield put(
        track("authorizeVerifiedClaim", {
          claim: claimNames,
          client_id: request.client_id,
          callback_url: request.callback_url
        })
    ...
    if (request.pushPermissions) {
      ...
      if (allowed) {
        ...
        const pushToken = yield call(
          createToken,
          request.target,
          pushPayload,
          { expiresIn: 52 * WEEK + DAY, issuer },
          `Allow ${counterparty || "service"} to send your push notifications`
        );
        yield put(updateActivity(request.id, { pushToken }));
        yield put(
          track("disclosureRequest Allowed Notifications", {
            client_id: request.client_id,
            callback_url: request.callback_url
          })
        );
    ...
    const token = yield call(
      ...
      request.target,
      payload,
      { expiresIn: TOKEN_VALIDITY, issuer },
      `Provide requested information to ${counterparty || "service"}`
    );
    if (request.client_id) {
      yield put(
        updateInteractionStats(request.target, request.client_id, "share")
      );
    ...
    yield put(
      track("disclosureRequest Authorized", {
        client_id: request.client_id,
        callback_url: request.callback_url,
        authorizedAt
      })
    ...
    console.log(" disclosure verification token is: ", token);
...
export function* authorizeAccount(request) {
  ...
  if (request.actType !== "none") {
    ...
    if (payload.nad === false)
      throw new Error("Requested account could not be create");
  }
