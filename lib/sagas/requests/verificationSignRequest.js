...
export function* authorize(request) {
  try {
    ...
    const issuer =
      request.riss || (!legacyClient && !request.target.match(/did:/))
        ? `did:uport:${request.target}`
        : request.target;
    const token = yield call(createToken, target, payload, {
      expiresIn: request.expiration ? request.expiration : false,
      issuer
    });
