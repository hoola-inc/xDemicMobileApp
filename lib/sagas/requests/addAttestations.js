...
import { ADD_ATTESTATION } from "xdemic/lib/constants/UportActionTypes";
import {
  updateActivity,
  removeActivity,
...
export function* handle(verified, jwt) {
  ...
  yield put(
    track("addAttestation", {
      claim: Object.keys(verified.claim)[0],
  ...
  if (params.attestations) {
    ...
    for (let token of params.attestations.split(/,/)) {
      try {
        ...
        yield put(
          track("addAttestation Invalid Verification", {
            callback_url: request.callback_url,
            client_id: request.client_id
          })
        );
    ...
    if (attestations.length === 0) {
      yield put(
        track("addAttestation Missing Verification", {
          callback_url: request.callback_url,
          client_id: request.client_id
        })
      );
    ...
    yield put(
      track("addAttestation Authorized", {
        callback_url: request.callback_url,
        client_id: request.client_id
      })
    );
    ...
    yield put(
      track("addAttestation Invalid Request", {
        callback_url: request.callback_url,
        client_id: request.client_id
      })
    );
