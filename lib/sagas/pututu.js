...
import {
  ...
  decryptMessage,
  decryptLegacy
} from "xdemic/lib/sagas/encryption";
...
export function* handleEncryptedMessage(
  { id, message, recipient, sender },
  popup
) {
  ...
  try {
    ...
    if (encrypted.version === ASYNC_ENC_ALGORITHM) {
      ...
      if (decrypted) {
        ...
        yield put(
          handleURL(message.url, { postback: true, popup, clientId: sender })
        );
      } else {
        console.log("COULD NOT DECRYPT MESSAGE");
      }
