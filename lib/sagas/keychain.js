...
import { storeIdentity } from "xdemic/lib/actions/uportActions";
import { saveError } from "xdemic/lib/actions/processStatusActions";
import {
  storeRootAddress,
  incIdentityIndex,
  incAccountIndex
} from "xdemic/lib/actions/HDWalletActions";
import { set, isSubset } from "mori";
import nacl from "xdemic/lib/vendor/nacl-fast";
...
// import bip39 from 'xdemic/lib/browserified/bip39'

// import { defaultNetwork } from 'xdemic/lib/utilities/networks'
...
export function* createSeed() {
  const { address, pubKey } = yield call(
    RNUportHDSigner.createSeed,
    DEFAULT_LEVEL
  );
...
export function* importSeed(phrase) {
  const { address, pubKey } = yield call(
    RNUportHDSigner.importSeed,
    phrase,
    DEFAULT_LEVEL
  );
  const publicKey = Buffer.from(pubKey, "base64").toString("hex");
  yield put(storeRootAddress(address));
  const publicEncKey = yield call(encryptionPublicKey, {
    idIndex: 0,
    actIndex: 0
  });
  return {
