...
export function* migrate(): any {
  ...
  const createOwnershipLink =
    (yield select(hasAttestations)) || (yield select(hasMainnetAccounts));

  let newRoot;
  if (yield call(hasWorkingSeed)) {
    const account = yield call(addressFor, 0, 0);
    const encPublicKey = yield call(encryptionPublicKey, {
      idIndex: 0,
      actIndex: 0
    });
  ...
  if (yield call(canSignFor, oldRoot)) {
    if (createOwnershipLink) {
      const link = yield call(createAttestationToken, oldRoot, newRoot, {
        owns: oldRoot
      });
      yield put(handleURL(`me.xdemic:req/${link}`, { popup: false }));
    }
  } else {
    yield put(
