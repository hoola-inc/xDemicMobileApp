...
import { reload, persist, loadedDB } from "xdemic/lib/actions/globalActions";
import {
  PERSIST_DB,
  FORCE_PERSIST_DB,
  RESET_DEVICE,
...
export const allStores = without(persistedReducers, "settings").map(
...
export function* distributePersistTasks() {
  ...
  return yield all(
    persistedReducers.map(reducer =>
      takeLatest(
        a => a.type === PERSIST_DB && a.reducer === reducer,
        debounchedPersistReducer
      )
    )
  );
}
