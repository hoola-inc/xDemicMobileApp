...
import {
  START_SEED_RECOVERY,
  RECOVERY_SUCCESS
} from "xdemic/lib/constants/RecoveryActionTypes";
import {
...
export function* identify({ address }) {
  ...
  const didMethod = address
    ? address.match(/^did:ethr:/)
      ? "ethr"
      : "uport"
    : undefined;
  if (segId && !optOut) {
  ...
  try {
    let properties = Object.keys(input).reduce((obj, key) => {
      if (input[key] instanceof Array) {
        // A1. Handle arrays first.
        input[key].forEach(function(rec) {
          obj[key] = scrubProperties(rec);
        }); // A2. Call recursively for each element.
      } else if (
        input[key] &&
        typeof input[key] === "object" &&
        input[key].constructor === Object
      ) {
        // B1. Handle objects second.
        switch (
          key // note: Add more keys as needed.
        ) {
      ...
      } else if (allowedProperties.includes(key) && input[key] !== undefined) {
        // C1. Handle anything else that is not an array or object
        key === "callback_url"
          ? (obj[key] = getHostName(input[key]))
          : (obj[key] = input[key]);
