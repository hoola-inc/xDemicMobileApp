...
export function fetchCourse(schoolDid) {
  console.log("fetchCourse(schoolDid) is: ", schoolDid);
  // `axios` function returns promise, you can use any ajax lib, which can
  // return promise, or wrap in promise ajax call
  if (schoolDid)
    return axios.get(`https://xdemic-api.herokuapp.com/courses/${schoolDid}`);
...
function* getSchool({ schoolDid }) {
  ...
  const { data } = yield call(fetchCourse, schoolDid);
...
function* courseSaga() {
  yield all([takeEvery(GET_COURSE, getSchool)]);
}
