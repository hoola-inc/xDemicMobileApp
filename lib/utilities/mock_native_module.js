...
//
module.exports = function MockNativeModule(
  signerAddress,
  privateKey,
  publicKey,
  root
  ...
  const naclutil = require("xdemic/lib/vendor/nacl-util");

  return {
    NativeModules: {
      NativeSignerModule: {
        signTx: (address, tx, message) =>
          new Promise((resolve, reject) => {
            if (address !== signerAddress)
              return reject(new Error("Unsupported address"));
            const signature = secp256k1.sign(
              Buffer.from(keccak256(Buffer.from(tx, "base64")), "hex"),
      ...
      NativeHDSignerModule: {
        signTx: (address, path, tx, message) =>
          new Promise((resolve, reject) => {
            if (address !== root)
              return reject(new Error("Unsupported address"));
            const signature = secp256k1.sign(
        ...
        importSeed: (seed, level) =>
          Promise.resolve({
            address: signerAddress,
            pubKey: Buffer.from(publicKey, "hex").toString("base64")
          }),
        addressForPath: (rootAddress, hdPath, prompt) =>
          Promise.resolve({
            address: signerAddress,
            pubKey: Buffer.from(publicKey, "hex").toString("base64")
          }),
        privateKeyForPath: (rootAddress, hdPath, prompt) =>
          Promise.resolve("Qgigj54O7CsQOhR5vLTfqSQyD3zmq/Gb8ukID7XvC3o="),
        ...
        randomBytes: (length, cb) =>
          cb(
            ...
            naclutil.decodeBase64(
              "Qgigj54O7CsQOhR5vLTfqSQyD3zmq/Gb8ukID7XvC3o="
            )
          )
      }
    },
