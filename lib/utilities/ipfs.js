...
function ipfsFetch(ipfsHash) {
  return new Promise((resolve, reject) => {
    RNFetchBlob.fetch("GET", `${ipfsUrl}/ipfs/${ipfsHash}`)
      .then(resolve)
      .catch(reject);
  });
}
  ...
  try {
    const granted = yield call(
      ...
      PermissionsAndroid.PERMISSIONS.READ_EXTERNAL_STORAGE,
      {
        ...
        message: "xDemic needs permission to read file "
      }
    );
...
export function* fetchUrlOrFile(url) {
  ...
  if (
    access &&
    (url.slice(0, 6) === "file://" || url.slice(0, 9) === "content://")
  ) {
