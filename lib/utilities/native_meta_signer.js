...
class NativeMetaSigner {
  constructor(
    ...
    root,
    relayAddress,
    whitelistOwner = DEFAULT_WHITELIST
  ) {
  ...
  signRawTx(rawTx, callback) {
    ...
    let hashInput =
      "1900" +
      stripHexPrefix(this.txRelayAddress) +
      stripHexPrefix(this.whitelistOwner) +
      padTo64(nonce) +
      to +
      data;
    ...
    delegateSigning(encodedHashInput, "Please sign transaction").then(
      signature => {
        let rawMetaSignedTx = txutils.functionTx(
          txRelayAbi,
          "relayMetaTx",
          [
            signature.v,
