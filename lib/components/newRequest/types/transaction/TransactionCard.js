...
import { defaultNetwork, networks } from "xdemic/lib/utilities/networks";
import EvilIcon from "react-native-vector-icons/EvilIcons";
import Ionicon from "react-native-vector-icons/Ionicons";
import AccountTileSmall from "xdemic/lib/components/shared/AccountTileSmall";
import metrics from "xdemic/lib/styles/metrics";
...
import {
  ...
  currentRequest,
  destinationProfile
} from "xdemic/lib/selectors/requests";
import { currentAddress } from "xdemic/lib/selectors/identities";
import { statusMessage } from "xdemic/lib/selectors/processStatus";
import {
  currentNetwork,
  networkSettingsForAddress
} from "xdemic/lib/selectors/chains";
...
import {
  ...
  cancelRequest,
  clearRequest
} from "xdemic/lib/actions/requestActions";
import { refreshBalance } from "xdemic/lib/actions/uportActions";
...
import { FAUCETS } from "xdemic/lib/utilities/networks";
    ...
    if (this.props.signerType === "KeyPair" && this.props.insufficientFunds) {
      return (
        ...
        >
          <View style={{ flex: 1 }}>
            <Text
              style={[textStyles.small, { color: "black", textAlign: "left" }]}
            >
              You will need to add funds to fully interact with this app. Add
              funds now?
            </Text>
            {this.props.faucet ? (
              <TouchableOpacity
                onPress={() => Linking.openURL(`https://${this.props.faucet}`)}
              >
                <Text
                  style={[
                    textStyles.small,
    ...
    if (this.state.advanced) {
      return (
        ...
        >
          ...
          >
            <TouchableOpacity onPress={() => this.toggleAdvanced()}>
              <Ionicon
                name="md-arrow-back"
                size={20}
                style={{ paddingLeft: 32, paddingBottom: 20 }}
              />
            </TouchableOpacity>
            <Text
              style={{
          ...
          <Text style={{ marginLeft: 24, marginBottom: 6, marginTop: 14 }}>
            {" "}
            Contract Details{" "}
          </Text>
          <View
          ...
          >
            <View>
              ...
              {this.props.request.abi && this.props.request.abi.args
                ? this.props.request.abi.args.map((arg, i) =>
                    FunctionParam(this.props.request.abi, arg, i)
                  )
                : null}
          ...
          <Text style={{ paddingTop: 13, paddingLeft: 24, paddingBottom: 17 }}>
            Contract Address
          </Text>
          <View
            style={{
      ...
      return (
        <View>
          ...
          >
            ...
            >
              <Text style={{ fontSize: 10, color: colors.grey74 }}>
                DETAILS
              </Text>
              <TouchableOpacity
                style={{ padding: 3 }}
                ...
                onPress={() => {
                  Linking.openURL(
                    networks[this.props.request.network] !== undefined
                      ? networks[this.props.request.network].explorerUrl +
                          "/tx/" +
                          this.props.request.txhash
                      : defaultNetwork.explorerUrl +
                          "/tx/" +
                          this.props.request.txhash
                  );
                }}
            ...
            {this.props.request.canceledAt ? (
            ...
            ) : !this.props.request.authorizedAt &&
              ...
              <CardStateBox
                ...
                state={
                  parseInt(this.props.request.status) === 1
                    ? "success"
                    : "error"
                }
                text={
        ...
        >
          ...
          {showUrl && (
            <View style={styles.url}>
              <TouchableOpacity
                onPress={() => Linking.openURL(this.props.client.url)}
              >
          ...
          <InteractionStats
            stats={
              typeof this.props.contact !== "undefined"
                ? this.props.contact.stats
                : {}
            }
...
const mapStateToProps = state => {
  ...
  const ethBalance =
    networkSettings.balance && networkSettings.balance.ethBalance
      ...
      : 0;
  const usdBalance =
    networkSettings.balance && networkSettings.balance.usdBalance
      ? networkSettings.balance.usdBalance
      : 0;
  const balanceWei =
    networkSettings.balance && networkSettings.balance.ethBalance
  ...
  const insufficientFunds = !(
    request &&
    request.txCost &&
    balanceWei.gt(new BN(request.txCost))
  );
