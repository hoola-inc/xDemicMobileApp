...
class CredentialExplorer extends React.Component<
  CredentialExplorerProps,
...
> {
  ...
  render() {
     ...
     */
    const normalisedClaimTree: ClaimTreeNormalised[] = normaliseClaimTree(
      this.props.claim
    );
            ...
            <Container flex={1}>
              {!isTopLevelSingleKey(this.props.claim, item.level) &&
                !item.hidden && (
                  ...
                  >
                    {!item.hasChildren
                      ? renderCrendentialItem(item.value)
                      : item.title}
                  </ListItem>
                )}
                    ...
                    return (
                      ...
                      >
                        {!listItem.isListItem &&
                          ...
                          collapsibleCredential(listItem.value)}
                        {listItem.isListItem && !listItem.hasChildren && (
                          <Container flexDirection={"row"} br={5} flex={1}>
                            ...
                            <Container flex={1}>
                              <ListItem last>
                                {renderCrendentialItem(listItem.value)}
                              </ListItem>
                            </Container>
                          </Container>
                        )}
